...
3. Academy can edit **only** the training start date and training end date on a coach's record. All other fields are read-only.
4. Super Admin and Admin can also edit the training fields (alongside everything else they already edit). HR does not get this power in v1.
5. A coach whose `today` falls between `trainingStartDate` and `trainingEndDate` gets a `🎓` badge next to their name everywhere a coach name renders — Employee Dashboard table, manpower-schedule planner, manpower-schedule update view.
...
- For Academy users (`isAcademy(userRole) === true`): all sections except the new Training section render read-only. The Edit button still enters edit mode, but only training fields are interactive. Save still calls `PUT /api/employees`.
- For Academy users: the user list on the left auto-filters to `role IN ("FT - Coach", "PT - Coach")`. Branch dropdown still works for narrowing.
- Form-level validation: if both dates are filled, end must be ≥ start. Save button disabled / inline error if not.
...
- For Academy users: same FT/PT-only filter as the user list.
...
if (isAcademy(session.user.role)) {
  where.role = { in: ["FT - Coach", "PT - Coach"] };  // server-side enforcement
}
```
...
| [app/components/UserManagement.tsx](../../../app/components/UserManagement.tsx) | New Training section in edit form; read-only mode for Academy on non-training fields; FT/PT-only client filter |
| [app/components/EmployeeTable.tsx](../../../app/components/EmployeeTable.tsx) | New "Training" column; FT/PT-only filter for Academy |
...
- Inside Employee Dashboard as Academy: list contains only FT/PT coaches across all branches; admins/HOD/BM rows do not appear.
- Open a coach record as Academy: only Training section is editable; setting start + end and saving works.
